...
from src.scene_vlm_only import Scene  # Use the new pure VLM scene
from src.utils import resize_image, get_pts_angle_aeqa
from src.query_vlm import query_vlm_for_response  # Use the new pure VLM query function
...
def main(cfg, start_ratio=0.0, end_ratio=1.0):
    ...
    express_bench_info = {}
    express_bench_info_file = os.path.join(
        cfg.output_dir, 
        f"express_bench_info_{start_ratio}_{end_ratio}.json"
    )
    
    if os.path.exists(express_bench_info_file):
        with open(express_bench_info_file, 'r') as f:
            express_bench_info = json.load(f)
    ...
    for question_idx, question_data in enumerate(questions_list):
        ...
        goal_position = question_data.get("goal_position", None)

        # load scene - for pure VLM, we don't pass detection models
        try:
            if 'scene' in locals():
                scene.close() 
        except:
        ...
        goal_dis = float('inf')
        if goal_position is not None and len(goal_position) == 3:
            try:
                path = habitat_sim.ShortestPath()
                path.requested_start = np.array(pts, dtype=np.float32)
                ...
                if found_path:
                    ...
                    logging.info(f"Goal distance: {goal_dis:.3f}")
                else:
                    logging.info(f"No path found to goal position")
            except Exception as e:
                logging.warning(f"Failed to compute goal distance: {e}")

        express_bench_info[question_id] = {
            "geodesic_distance": geodesic_distance,
            "goal_dis": goal_dis,
            "final_position": pts.tolist(),
        ...
        logging.info(f"EXPRESS-Bench info: geodesic_dist={geodesic_distance:.3f}, goal_dis={goal_dis:.3f}")
        scene.print_scene_graph()

        # update the saved results after each episode
        logger.save_results()
        with open(express_bench_info_file, 'w') as f:
            json.dump(express_bench_info, f, indent=2)
    ...
    logging.info(f"EXPRESS-Bench info saved to: {express_bench_info_file}")


if __name__ == "__main__":
    # Get config path
    parser = argparse.ArgumentParser()
    parser.add_argument("-cf", "--cfg_file", help="cfg file path", default="/.../Pred-EQA/cfg/eval_aeqa.yaml", type=str)
