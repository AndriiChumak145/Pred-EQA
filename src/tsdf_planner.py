...
class TSDFPlanner(TSDFPlannerBase):
    ...
    def __init__(
        ...
        # about navigation
        self.max_point: Optional[Union[Frontier, SnapShot]] = (
            None  # the frontier/snapshot the agent chooses
        )
        self.target_point: Optional[np.ndarray] = (
    ...
    def update_frontier_map(
        ...
        if len(frontier_areas) == 0:
            # this happens when there are stairs on the floor, and the planner cannot handle this situation
            # For VLM-only approach, we should handle this more gracefully
            logging.warning("Warning in update_frontier_map: frontier area size is 0")
            # Instead of returning False, we can create a minimal frontier or continue with empty frontiers
            # This allows the exploration to continue even when no frontiers are detected initially
            self.frontiers = []  # Clear existing frontiers
            return True  # Return True to continue exploration
        if len(frontier_edge_areas) == 0:
            # this happens rather rarely
            logging.warning("Warning in update_frontier_map: frontier edge area size is 0")
            self.frontiers = []
            return True  # Return True to continue exploration instead of failing
        ...
        for i, frontier in enumerate(self.frontiers):
            ...
            # TODO update all frontiers
            # if frontier.image is None:
            # frontier.image = None
            # frontier.feature = None
            if frontier.image is None:
                ...
                if save_frontier_image and eps_frontier_dir is not None:
                    if not os.path.exists(eps_frontier_dir):
                        logging.error(f"Error in update_frontier_map: {eps_frontier_dir} does not exist")
                        continue
                    plt.imsave(
    ...
    def set_next_navigation_point(
        ...
        if self.max_point is not None or self.target_point is not None:
            ...
            logging.error(
                "Error in set_next_navigation_point: the next point is already set: %s, %s",
                self.max_point, self.target_point
            )
            return False
        pts = pos_habitat_to_normal(pts)
        cur_point = self.normal2voxel(pts)
        ...
        if type(choice) == SnapShot:
            # For pure VLM approach, the snapshot.cluster might be empty
            # In this case, we use the obs_point as the position for navigation
            if len(choice.cluster) == 0:
            ...
            while (
                ...
                if try_count > 1000:
                    logging.error(
                        "Error in set_next_navigation_point: cannot find a proper next point"
                    )
                    return False
            ...
            logging.error(
                "Error in find_next_pose_with_path: wrong choice type: %s",
                type(choice)
            )
            return False
    ...
    def agent_step(
        ...
        if self.max_point is None or self.target_point is None:
            logging.error(
                "Error in agent_step: max_point or next_point is None: %s, %s",
                self.max_point, self.target_point
            )
        ...
        if dist > max_dist_from_cur:
            ...
            if path_to_target is not None:
                ...
                logging.info(
                    "pathfinder cannot find a path from %s to %s, just go to a point between them",
                    cur_point[:2], self.target_point
                )
                ...
                while (
                    ...
                    if try_count > 1000:
                        logging.error(
                            "Error in agent_step: cannot find a proper next point"
                        )
                        return (None, None, None, None)
        ...
        if save_visualization:
            ...
            try:
                ...
                for key, snapshot in snapshots.items():
                    ...
                    if obj_angles and max(obj_angles) - min(obj_angles) > 180:
                        obj_angles = [
                ...
                for frontier in self.frontiers:
                    ...
                    if type(self.max_point) == Frontier and frontier == self.max_point:
                        ...
                        # Add edge to the arrow by creating a slightly larger arrow with red color behind
                        arrow_edge = FancyArrowPatch(
                            posA=(frontier.position[1], frontier.position[0]),
                            posB=(frontier.position[1] + dy, frontier.position[0] + dx),
                            arrowstyle=f"Simple, tail_width={0.5}, head_width={7}, head_length={7}",  # slightly larger
                            linewidth=5,
                            color="red",
                            mutation_scale=1,
                        )
                        ax1.add_patch(arrow_edge)  # Add the red arrow first (as background)
                        ax1.add_patch(arrow)  # Then add the original arrow on top
                    else:
                        ax1.add_patch(arrow)
        ...
        if self._voxel_size is None or self._vol_origin is None:
            logging.error("Error: _voxel_size or _vol_origin is None")
